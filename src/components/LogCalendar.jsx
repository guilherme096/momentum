      ...
      <Dialog open={isOpen} onClose={() => setIsOpen(false)} className="relative z-50 ">
        ...
        <div className="fixed inset-0 flex w-screen items-center justify-center p-4">
          <DialogPanel className="w-full max-w-lg max-h-[80%] overflow-y-scroll border bg-white p-8 rounded-lg">
            ...
            <div className='mt-6'>
                <h2 className="text-xl font-bold inline">Date: </h2><p className="inline">{currentLog?.date}</p>
            </div>
            {currentLog?.duration &&
            <div>
                <h2 className="text-xl font-bold inline">Duration: </h2><p className="inline">{currentLog?.duration}</p>
            </div>
            }
                ...
                {currentLog?.exercises_done.map((exercies_logged) => {
                    return(
                        <ExerciseCard
                            ...
                            name={exercises.find((exercise) => exercise.id === exercies_logged.id).name}
