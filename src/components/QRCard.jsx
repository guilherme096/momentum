  ...
  return (
    <>
      <div className="bg-transparent h-fit w-fit mx-auto rounded-md">
        <div
          className={`card w-full h-32 bg-transparent rounded-lg mx-auto flex  ${timerActive ? "flex-col" : "flex-row"} justify-between align-middle items-center`}
        >
          <motion.div
            layoutId="1"
            ref={qrContainer}
            className={`bg-qr-code min-w-32 bg-no-repeat bg-center bg-contain h-full w-full ${timerActive ? "" : "blur-sm"} bg-base-100`}
            onClick={toggleModal}
          ></motion.div>
          <button
            ref={qrButton}
            className="btn btn-outline ml-8 shadow-md"
            onClick={handleStart}
          >
            Show QR Code
          </button>
          {timerActive && (
            <>
              <div className="mt-4" ref={timerContainer}>
                <Countdown
                  isActive={timerActive}
                  completedCallback={handleStop}
                />
              </div>
        ...
        {isModalOpen && (
          ...
          >
            ...
            >
              <motion.div
                layoutId="1"
                className="bg-qr-code min-w-64 min-h-64 max-w-64 max-h-64 bg-no-repeat bg-center bg-contain relative flex flex-col justify-end"
              ></motion.div>
            </motion.div>
          </motion.div>
        )}
